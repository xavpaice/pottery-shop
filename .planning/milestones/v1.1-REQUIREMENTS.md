...
- [x] **TLS-01**: User can enable Let's Encrypt mode (`ingress.tls.mode: letsencrypt`) — HTTP-01 ACME ClusterIssuer, staging ACME URL by default, production opt-in via `ingress.tls.acme.production: true`
- [x] **TLS-02**: User can enable self-signed mode (`ingress.tls.mode: selfsigned`) — two-step CA bootstrap: SelfSigned ClusterIssuer → CA cert → CA ClusterIssuer → app cert
...
- [x] **CI-06**: `integration-test.yml` includes a cert-manager v1.20.2 pre-install step (mirrors existing CNPG pre-install pattern)
...
| TLS-01 | Phase 4 | Satisfied |
| TLS-02 | Phase 4 | Satisfied |
| TLS-03 | Phase 3 | Satisfied |
| CI-04 | Phase 5 | Satisfied |
| CI-05 | Phase 5 | Satisfied |
| CI-06 | Phase 4 | Satisfied |

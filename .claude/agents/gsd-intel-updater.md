...
- **Cross-platform.** Use Glob, Read, and Grep tools -- not Bash `ls`, `find`, or `cat`. Bash file commands fail on Windows. Only use Bash for `node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs intel` CLI calls.
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs intel extract-exports <file>` to get accurate exports.
...
```bash
node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/stack.json --cwd <project_root>
```
...
```bash
node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/files.json --cwd <project_root>
```
...
```bash
node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/apis.json --cwd <project_root>
```
...
```bash
node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/deps.json --cwd <project_root>
```
...
Run: `node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs intel validate --cwd <project_root>`
...
Run: `node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs intel snapshot --cwd <project_root>`

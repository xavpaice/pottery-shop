...
```bash
PHASE_INFO=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```

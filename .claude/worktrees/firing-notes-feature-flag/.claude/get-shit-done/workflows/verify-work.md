...
<template>
@/shared/pottery-shop/.claude/get-shit-done/templates/UAT.md
</template>
...
```bash
INIT=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```
UI_PHASE_FLAG=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
Read and follow `/shared/pottery-shop/.claude/get-shit-done/workflows/transition.md`.
...
```bash
node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" audit-open --json 2>/dev/null
```
...
- Follow @/shared/pottery-shop/.claude/get-shit-done/workflows/diagnose-issues.md

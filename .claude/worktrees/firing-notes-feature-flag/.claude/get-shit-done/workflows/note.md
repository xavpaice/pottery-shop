...
- **Global scope**: `/shared/pottery-shop/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/shared/pottery-shop/.claude/notes/`)
...
2. Glob `/shared/pottery-shop/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/shared/pottery-shop/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/shared/pottery-shop/.claude/notes/` — works in any directory

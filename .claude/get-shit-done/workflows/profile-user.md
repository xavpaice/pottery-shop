...
Key references:
- @/shared/pottery-shop/.claude/get-shit-done/references/ui-brand.md (display patterns)
- @/shared/pottery-shop/.claude/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @/shared/pottery-shop/.claude/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="/shared/pottery-shop/.claude/get-shit-done/USER-PROFILE.md"
...
```bash
cp "/shared/pottery-shop/.claude/get-shit-done/USER-PROFILE.md" "/shared/pottery-shop/.claude/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at /shared/pottery-shop/.claude/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `/shared/pottery-shop/.claude/get-shit-done/references/user-profiling.md`
...
Reference: @/shared/pottery-shop/.claude/get-shit-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to /shared/pottery-shop/.claude/get-shit-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global CLAUDE.md" -- "Add profile to /shared/pottery-shop/.claude/CLAUDE.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at /shared/pottery-shop/.claude/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd-dev-preferences at /shared/pottery-shop/.claude/commands/gsd/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node /shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to /shared/pottery-shop/.claude/CLAUDE.md"
...
```bash
BACKUP_PATH="/shared/pottery-shop/.claude/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    /shared/pottery-shop/.claude/get-shit-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd-dev-preferences   /shared/pottery-shop/.claude/commands/gsd/dev-preferences.md
  ✓ CLAUDE.md section       ./CLAUDE.md
  ✓ Global CLAUDE.md        /shared/pottery-shop/.claude/CLAUDE.md
```

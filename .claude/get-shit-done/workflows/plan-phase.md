...
@/shared/pottery-shop/.claude/get-shit-done/references/ui-brand.md
@/shared/pottery-shop/.claude/get-shit-done/references/revision-loop.md
@/shared/pottery-shop/.claude/get-shit-done/references/gate-prompts.md
...
```bash
INIT=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
...
TDD_MODE=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.tdd_mode 2>/dev/null || echo "false")
```

When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@/shared/pottery-shop/.claude/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
```bash
PHASE_INFO=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
AI_PHASE_CFG=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
```bash
PHASE_DESC=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `/shared/pottery-shop/.claude/get-shit-done/templates/VALIDATION.md`
...
SECURITY_ASVS=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```bash
PHASE_SECTION=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
2. Disable Nyquist with the exact command:
   `node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
Check config:
```bash
PATTERN_MAPPER_CFG=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.pattern_mapper --default true 2>/dev/null)
```
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/shared/pottery-shop/.claude/get-shit-done/references/tdd.md to all eligible tasks:
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.plan_bounce_passes --default 2)
BOUNCE_SCRIPT=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.plan_bounce_script)
```
...
```bash
node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
fi
```

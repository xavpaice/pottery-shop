...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
```bash
ARCHIVE=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**If doesn't exist:** Create from template at `/shared/pottery-shop/.claude/get-shit-done/templates/retrospective.md`.
...
**Commit:**
```bash
node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "1")
...
Detect base branch:
```bash
BASE_BRANCH=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" config-get git.base_branch 2>/dev/null || echo "")
...
```bash
node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```

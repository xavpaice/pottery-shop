...
<execution_context>
@/shared/pottery-shop/.claude/get-shit-done/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" frontmatter get .planning/quick/{dir}/SUMMARY.md --field status 2>/dev/null
  ```
...
5. Load context via:
   ```bash
   node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" init quick
   ```
...
Execute the quick workflow from @/shared/pottery-shop/.claude/get-shit-done/workflows/quick.md end-to-end.

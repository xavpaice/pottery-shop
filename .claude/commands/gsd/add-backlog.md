...
2. **Find next backlog number:**
   ```bash
   NEXT=$(node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" phase next-decimal 999 --raw)
   ```
...
5. **Commit:**
   ```bash
   node "/shared/pottery-shop/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: add backlog item ${NEXT} — ${ARGUMENTS}" --files .planning/ROADMAP.md ".planning/phases/${NEXT}-${SLUG}/.gitkeep"
   ```
